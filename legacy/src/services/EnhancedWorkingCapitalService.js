...
class EnhancedWorkingCapitalService {
  ...
  async fetchSalesData(period) {
    ...
    // Fallback: Manufacturing industry estimates (Â£40M annual revenue)
    const estimated = {
  ...
  generateOptimizationRecommendations(data) {
    ...
    if (cashConversionCycle > benchmarks.cashConversionCycle.target) {
      recommendations.push({
        ...
        potentialImpact: `Â£${Math.round((cashConversionCycle - benchmarks.cashConversionCycle.target) * requirements.operational.amount / 30)}K in freed capital`,
