 ...
 */
router.get(_'/organizations', async _(req, res) => {
  try {
 ...
 */
router.get(_'/contacts', async _(req, res) => {
  try {
    const accessToken = req.headers.authorization?.replace('Bearer ', '') || req.session?.xeroTokens?.access_token;
 ...
 */
router.post(_'/contacts', async _(req, res) => {
  try {
    const accessToken = req.headers.authorization?.replace('Bearer ', '') || req.session?.xeroTokens?.access_token;
 ...
 */
router.get(_'/invoices', async _(req, res) => {
  try {
    const accessToken = req.headers.authorization?.replace('Bearer ', '') || req.session?.xeroTokens?.access_token;
    const { tenantId, page = 1, limit = 100 } = req.query;
 ...
 */
router.post(_'/invoices', async _(req, res) => {
  try {
    const accessToken = req.headers.authorization?.replace('Bearer ', '') || req.session?.xeroTokens?.access_token;
 ...
 */
router.get(_'/items', async _(req, res) => {
  try {
    const accessToken = req.headers.authorization?.replace('Bearer ', '') || req.session?.xeroTokens?.access_token;
    const { tenantId, page = 1, limit = 100 } = req.query;
 ...
 */
router.post(_'/items', async _(req, res) => {
  try {
    const accessToken = req.headers.authorization?.replace('Bearer ', '') || req.session?.xeroTokens?.access_token;
