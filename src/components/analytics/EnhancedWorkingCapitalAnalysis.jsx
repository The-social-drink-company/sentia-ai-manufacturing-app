...
const EnhancedWorkingCapitalAnalysis = () => {
  ...
  const projectionData = useMemo(() => {
    ...
    for (let i = 0; i <= 12; i++) {
      ...
      const variabilityFactor = 1 + ((Math.random() - 0.5) * parameters.cashFlow.variability) / 100;
    ...
    if (autoRefresh) {
      const interval = setInterval(() => {
        ...
        setParameters(prev => ({
          ...
          cashFlow: {
            ...prev.cashFlow,
            variability: Math.max(5, Math.min(25, prev.cashFlow.variability + (Math.random() - 0.5) * 2))
          }
        }));
