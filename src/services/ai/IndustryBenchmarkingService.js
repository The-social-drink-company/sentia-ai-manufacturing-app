...
Industry: ${industry}
Annual Revenue: £${revenue?.toLocaleString() || 'Not specified'}
...
Format the response as structured data that can be easily parsed and used in financial analysis.`;
  ...
  async getAnthropicBenchmarks(params) {
    ...
    const prompt = `Analyze working capital benchmarks for ${industry} companies in ${region} with approximately £${revenue?.toLocaleString()} revenue and ${employees} employees.
...
Focus on actionable executive insights with specific numerical benchmarks.`;
  ...
  parseOpenAIResponse(response) {
    try {
      ...
      const revenuePerEmployeeMatch = response.match(/Revenue per Employee[:\s]*£?(\d+,?\d*)/i);
    ...
    try {
      ...
      return {
        ...
        financial: {
          ...
          revenuePerEmployee: { average: 175000, bestInClass: 275000, unit: '£' }
        },
        insights: this.extractInsights(response),
        source: 'anthropic',
        generatedAt: new Date().toISOString()
  ...
  getFallbackBenchmarks(params) {
    return {
      ...
      financial: {
        ...
        revenuePerEmployee: { average: 150000, bestInClass: 225000, unit: '£' }
      },
      insights: [
