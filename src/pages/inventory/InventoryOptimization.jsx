...
function EOQCalculationCard({ sku, eoqData }) {
  const { eoq, annualDemand, orderingCost, holdingCost, totalCost, numberOfOrders } = eoqData

  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">EOQ Calculation - {sku}</h3>

      {/* Formula Display */}
      <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm font-medium text-gray-700 mb-2">Economic Order Quantity Formula:</p>
        <p className="font-mono text-lg text-center">EOQ = √((2 × D × S) / H)</p>
...
function SafetyStockCard({ sku, safetyStockData }) {
  const { safetyStock, serviceLevel, zScore, leadTime, demandVariability } = safetyStockData

  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">Safety Stock Calculation - {sku}</h3>

      {/* Formula Display */}
      <div className="mb-4 p-4 bg-orange-50 border border-orange-200 rounded-lg">
        <p className="text-sm font-medium text-gray-700 mb-2">Safety Stock Formula:</p>
        <p className="font-mono text-lg text-center">SS = Z × σ × √LT</p>
        <div className="grid grid-cols-3 gap-2 mt-3 text-xs text-gray-600">
...
function ReorderPointCard({ sku, ropData }) {
  const { reorderPoint, leadTimeDemand, safetyStock, currentStock, daysUntilReorder } = ropData

  const needsReorder = currentStock <= reorderPoint

  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">Reorder Point - {sku}</h3>
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p
              className={`text-3xl font-bold ${needsReorder ? 'text-red-600' : 'text-purple-600'}`}
            >
              {reorderPoint} units
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className={`p-3 rounded ${needsReorder ? 'bg-red-50' : 'bg-green-50'}`}>
          ...
          <p
            className={`text-lg font-semibold ${needsReorder ? 'text-red-600' : 'text-green-600'}`}
          >
            {currentStock} units
          </p>
        </div>
...
function ForecastIntegrationPanel({ forecastData }) {
  if (!forecastData) {
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Forecast Integration</h3>
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Forecast Integration</h3>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <ComposedChart data={forecastData.data}>
          ...
          <XAxis
            dataKey="date"
            tickFormatter={date => new Date(date).toLocaleDateString('en-GB', { day: '2-digit' })}
          />
          <YAxis />
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {results.map(result => (
              <tr key={result.sku} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{result.eoq}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  {result.safetyStock}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
...
export default function InventoryOptimization() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Configuration Panel */}
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Optimization Configuration</h3>
            ...
            <input
              ...
              onChange={e =>
                setOptimizationConfig({
                  ...optimizationConfig,
                  serviceLevel: Number(e.target.value),
                })
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={e =>
                setOptimizationConfig({
                  ...optimizationConfig,
                  holdingCostPercent: Number(e.target.value),
                })
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              min="0"
      ...
      {optimizationResults.length === 0 && (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          ...
          <p className="text-gray-600 mb-4">
            Select SKUs and run optimization to see EOQ, safety stock, and reorder point
            calculations
          </p>
          <button
            onClick={() => runOptimizationMutation.mutate(optimizationConfig)}
