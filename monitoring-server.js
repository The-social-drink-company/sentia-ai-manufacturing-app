...
class EnterpriseMonitoringDashboard {
  ...
  async checkSelfHealingStatus() {
    try {
      ...
      const exists = await fs.access(fixLogPath).then(() => true).catch(() => false);
  ...
  calculateSystemHealth() {
    ...
    else if (this.systemMetrics.testingSuite.status === 'running') score += weights.testingSuite * 0.8;
    
    // Self-healing health
    if (this.systemMetrics.selfHealing.status === 'active') score += weights.selfHealing;
    
    // API health
    const healthyApis = Object.values(this.systemMetrics.apiHealth).filter(api => api.status === 'healthy').length;
  ...
  parseTestResults(logLines) {
    ...
    for (const line of logLines.slice(-50)) { // Check last 50 lines
    ...
    results.status = results.failed === 0 && results.total > 0 ? 'passing' : results.failed > 0 ? 'failing' : 'unknown';
  ...
  async checkDeploymentStatus() {
    try {
      ...
      const exists = await fs.access(deployLogPath).then(() => true).catch(() => false);
