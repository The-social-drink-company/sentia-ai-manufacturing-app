        ...
        scenarios: {
          baseline: {
            ...
            summary: {
              ...
              breachMonths: monthlyProjections.filter(m => m.ending_cash < 500000).length, // Â£500k minimum for manufacturing
          ...
          <div className="sentia-grid sentia-grid-3">
              ...
              <div className="sentia-card-content">
                <div className="sentia-metric">
                  <span className="sentia-metric-value">
                    Â£{projectionData.scenarios.baseline.summary.totalCashIn.toLocaleString()}
                  </span>
            ...
            <div className="sentia-card">
              ...
              <div className="sentia-card-content">
                <div className="sentia-metric">
                  <span className="sentia-metric-value">
                    Â£{projectionData.scenarios.baseline.summary.totalCashOut.toLocaleString()}
                  </span>
                  <span className="sentia-metric-label">
                    Over {projectionParams.horizonMonths} months
                  </span>
              ...
              <div className="sentia-card-content">
                <div className="sentia-metric">
                  <span className={`sentia-metric-value ${projectionData.scenarios.baseline.summary.netCashFlow >= 0 ? 'sentia-positive' : 'sentia-negative'}`}>
                    Â£{projectionData.scenarios.baseline.summary.netCashFlow.toLocaleString()}
                  </span>
                  <span className="sentia-metric-label">
                    ...
                    {projectionData.scenarios.baseline.projections.map((projection, index) => {
                      ...
                      return (
                        <tr key={projection.id}>
                          <td>{monthName}</td>
                          <td>Â£{projection.openingCash ? projection.openingCash.toLocaleString() : '0'}</td>
                          <td className="sentia-positive">Â£{projection.cash_in.toLocaleString()}</td>
                          <td className="sentia-negative">Â£{projection.cash_out.toLocaleString()}</td>
                          <td className={projection.net_change >= 0 ? 'sentia-positive' : 'sentia-negative'}>
                            Â£{projection.net_change.toLocaleString()}
                          </td>
                          <td className={isLowCash ? 'sentia-negative' : 'sentia-positive'}>
                            Â£{projection.ending_cash.toLocaleString()}
                          </td>
                          <td>
                            {isLowCash && (
                ...
                <div className="sentia-alert sentia-alert-warning">
                  <div className="sentia-alert-content">
                    ...
                    <div className="sentia-alert-details">
                      Minimum cash level: Â£{projectionData.scenarios.baseline.summary.minCash.toLocaleString()}
                    </div>
                  </div>
                </div>
              </div>
