          ...
          <div className="mt-6 rounded-md bg-white p-4 text-left shadow-sm">
            ...
            <div className="space-y-2">
              {shopifyStatus.stores.map((store) => (
                <div key={store.id} className="flex items-center justify-between p-3 rounded-md border border-slate-200">
                  <div className="flex-1">
                    <div className="font-medium text-sm text-slate-900">{store.name}</div>
                    <div className="text-xs text-slate-500 mt-1">
                      {store.region.toUpperCase()}
                      {store.lastSync && ` • Last sync: ${new Date(store.lastSync).toLocaleTimeString()}`}
                    </div>
                  </div>
                  {store.isActive ? (
        ...
        <div className="mt-6 rounded-md bg-blue-100 p-4 text-left">
          ...
          <ul className="space-y-2 text-sm text-blue-700">
            <li className="flex items-start gap-2">
              ...
              <div>
                <code className="font-mono text-xs bg-blue-200 px-2 py-1 rounded">SHOPIFY_UK_SHOP_DOMAIN</code>
                <span className="text-xs text-blue-600 ml-2">- UK/EU store domain (e.g., sentia-uk-eu)</span>
              </div>
            </li>
            <li className="flex items-start gap-2">
              <ExclamationCircleIcon className="h-4 w-4 flex-shrink-0 mt-0.5" />
              <div>
                <code className="font-mono text-xs bg-blue-200 px-2 py-1 rounded">SHOPIFY_UK_ACCESS_TOKEN</code>
                <span className="text-xs text-blue-600 ml-2">- UK/EU access token (starts with shpat_)</span>
              </div>
            </li>
            <li className="flex items-start gap-2">
              <ExclamationCircleIcon className="h-4 w-4 flex-shrink-0 mt-0.5" />
              <div>
                <code className="font-mono text-xs bg-blue-200 px-2 py-1 rounded">SHOPIFY_US_SHOP_DOMAIN</code>
                <span className="text-xs text-blue-600 ml-2">- USA store domain (e.g., sentia-usa)</span>
              </div>
            </li>
            <li className="flex items-start gap-2">
              <ExclamationCircleIcon className="h-4 w-4 flex-shrink-0 mt-0.5" />
              <div>
                <code className="font-mono text-xs bg-blue-200 px-2 py-1 rounded">SHOPIFY_US_ACCESS_TOKEN</code>
                <span className="text-xs text-blue-600 ml-2">- USA access token (starts with shpat_)</span>
              </div>
            </li>
          </ul>
        </div>
          ...
          <ol className="space-y-3 text-sm text-slate-700">
            ...
            <li className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Enable scopes: <code className="font-mono text-xs bg-slate-100 px-1">read_orders</code>,{' '}
        ...
        <div className="mt-6 rounded-md bg-gradient-to-br from-green-50 to-blue-50 p-4 text-left">
          ...
          <ul className="space-y-2 text-xs text-slate-700">
            <li className="flex items-start gap-2">
              ...
              <span><strong>Real-time Sales Data:</strong> Live order tracking from UK/EU and USA stores</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>Commission Tracking:</strong> Automatic 2.9% Shopify transaction fee calculations</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>Regional Breakdown:</strong> Compare UK (GBP) vs USA (USD) performance</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>Product Analytics:</strong> Top sellers, units sold, revenue by SKU</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>Auto-Sync:</strong> Data refreshes every 15 minutes automatically</span>
            </li>
          </ul>
        </div>
