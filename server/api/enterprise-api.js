...
router.get('/dashboard/summary', async (req, res) => {
  try {
    ...
    if (xeroClient && xeroClient.tenantIds?.length > 0) {
      ...
      const cashFlow = await xeroClient.accountingApi.getReportBalanceSheet(xeroTenantId, null)
    ...
    if (shopifyClient) {
      ...
      inventoryData = {
        value:
          inventoryLevels.body?.inventory_levels?.reduce(
            (sum, item) => sum + item.available * (item.cost || 0),
            0
          ) || 0,
        turnover: 0, // Calculate from sales/inventory
        skuCount: products.body?.products?.length || 0,
        lowStock:
          inventoryLevels.body?.inventory_levels?.filter(item => item.available < 10).length || 0,
      }
    }
...
router.get('/financial/working-capital', async (req, res) => {
  try {
    ...
    const cashFlowStatement =
      await xeroClient.accountingApi.getReportCashflowStatement(xeroTenantId)
    ...
    res.json({
      ...
      workingCapital:
        (balanceSheet.body?.reports?.[0]?.rows?.[1]?.cells?.[1]?.value || 0) -
