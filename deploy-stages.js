...
const stages = {
  '1': {
    ...
    excludeComponents: ['AI', 'Executive', 'WorkingCapital', 'analytics', 'admin', 'quality', 'inventory', 'mobile', 'automation'],
    memoryLimit: 2048,
...
function createStageViteConfig() {
  ...
  const excludePattern = currentStage.excludeComponents.length > 0
...
function createStageMarker() {
  ...
  const marker = {
    ...
    nextStage: parseInt(DEPLOYMENT_STAGE) < 4 ? (parseInt(DEPLOYMENT_STAGE) + 1).toString() : 'complete'
  };
