    ...
    <div className={`p-4 bg-white border rounded-lg ${className}`}>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <div className="text-xs text-gray-500 space-y-1">
          <div>â€¢ Password history: Last {policy?.preventReuse || 5} passwords cannot be reused</div>
          <div>â€¢ Password expires after {(policy?.maxAge || 0)} days</div>
          <div>â€¢ Maximum length: {(policy?.maxLength || 0)} characters</div>
        </div>
      </div>
    </div>
  )
