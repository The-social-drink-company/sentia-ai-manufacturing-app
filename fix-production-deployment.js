...
async function runDeploymentFixes() {
  ...
  try {
    ...
    // Count files
    const jsFiles = fs.existsSync(jsPath) ? fs.readdirSync(jsPath).filter(f => f.endsWith('.js')).length : 0;
    const cssFiles = fs.existsSync(assetsPath) ? fs.readdirSync(assetsPath).filter(f => f.endsWith('.css')).length : 0;
...
`;
  ...
  if (successful === total) {
    ...
    console.log('\nNext steps:');
    console.log('1. Commit changes: git add . && git commit -m "Fix: Production deployment MIME type and CSP issues"');
