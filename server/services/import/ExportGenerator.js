...
async function drawPDFTable(doc, data, columns, columnWidth, fontSize, includeHeaders) {
  ...
  if (includeHeaders) {
    ...
    // Header background
    doc.rect(startX, startY, columnWidth * columns.length, headerHeight)
      .fill('#4472C4');
  ...
  data.forEach((row, rowIndex) => {
    ...
    if (rowIndex % 2 === 0) {
      doc.rect(startX, startY, columnWidth * columns.length, rowHeight)
        .fill('#F8F9FA')
        .fillColor('#000000');
    }
...
async function listExports(options = {}) {
  try {
    ...
    const exports = await Promise.all(
      files.map((filename) => getExportMetadata(filename))
    );
