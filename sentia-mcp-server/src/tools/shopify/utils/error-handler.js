 ...
 * 
 * Comprehensive error handling for Shopify API operations with retry logic,
...
export class ShopifyErrorHandler {
  constructor(options = {}) {
    this.options = {
      ...
      rateLimitBackoff: options.rateLimitBackoff || 2000, // 2 seconds
    ...
    logger.info('Shopify Error Handler initialized', {
  ...
  /**
   * Handle and categorize Shopify API errors
   */
  ...
  analyzeError(error) {
    ...
    let statusCode = null;
    let shopifyErrorCode = null;
    let retryAfter = null;

    // Handle Shopify API-specific errors
    if (error.response) {
      ...
      if (error.response.data) {
        ...
        // Shopify errors format
        if (errorData.errors) {
          if (typeof errorData.errors === 'string') {
            shopifyErrorCode = errorData.errors;
          } else if (Array.isArray(errorData.errors)) {
            shopifyErrorCode = errorData.errors[0];
          } else if (typeof errorData.errors === 'object') {
            shopifyErrorCode = Object.values(errorData.errors)[0];
          }
        }
      }
      ...
      if (error.response.headers) {
        const rateLimitRemaining = error.response.headers['x-shopify-api-call-limit'];
      ...
      switch (statusCode) {
        ...
        case 403:
          errorType = 'FORBIDDEN';
          severity = 'ERROR';
          userMessage = 'Access denied. Check your API permissions and scopes.';
          break;
        ...
        case 406:
          ...
          severity = 'WARNING';
          userMessage = 'Request format not acceptable. Check Content-Type headers.';
          break;
          
        case 422:
          errorType = 'VALIDATION_ERROR';
          severity = 'WARNING';
          userMessage = `Validation error: ${shopifyErrorCode || 'Please check your data format'}`;
          break;
        ...
        case 504:
          ...
          severity = 'ERROR';
          isRetryable = true;
          userMessage = 'Shopify server error. Please try again in a few moments.';
          break;
          
        default:
          errorType = 'HTTP_ERROR';
          severity = 'ERROR';
          userMessage = `HTTP error ${statusCode}: ${shopifyErrorCode || 'Unknown error'}`;
      }
  ...
  createEnhancedError(errorInfo, context, metadata) {
    ...
    enhancedError.shopifyErrorCode = errorInfo.shopifyErrorCode;
  ...
  logError(error, errorInfo) {
    const logData = {
      ...
      shopifyErrorCode: error.shopifyErrorCode,
    ...
    switch (errorInfo.severity) {
      case 'WARNING':
        logger.warn('Shopify API warning', logData);
        break;
      case 'ERROR':
        logger.error('Shopify API error', logData);
        break;
      default:
        logger.error('Shopify API error', logData);
    }
  ...
  async withRetry(operation, context = '', options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          logger.info('Shopify operation succeeded after retry', {
            context,
            attempt,
        ...
        logger.warn('Shopify operation failed, retrying', {
          context,
          attempt: attempt + 1,
          maxRetries,
          delay,
  ...
  updateCircuitBreaker(context, errorInfo) {
    if (!errorInfo.isRetryable && errorInfo.severity === 'ERROR') {
      ...
      if (newCount >= this.options.circuitBreakerThreshold) {
        ...
        logger.warn('Shopify circuit breaker opened', {
          context,
          errorCount: newCount,
        ...
        setTimeout(() => {
          ...
          logger.info('Shopify circuit breaker set to half-open', { context });
  ...
  updateRateLimitState(context, error) {
    ...
    if (error.response?.headers) {
      const callLimit = error.response.headers['x-shopify-api-call-limit'];
      const retryAfter = error.response.headers['retry-after'];
      
      rateLimitInfo.callLimit = callLimit;
  ...
  resetCircuitBreaker(context) {
    ...
    if (this.circuitState.has(key)) {
      ...
      logger.info('Shopify circuit breaker reset', { context });
    }
  }
  ...
  clearErrorHistory() {
    ...
    logger.info('Shopify error handler history cleared');
  ...
  getRecommendedAction(errorType) {
    const recommendations = {
      ...
      'UNAUTHORIZED': 'Check API credentials and refresh tokens if needed',
      'FORBIDDEN': 'Verify API permissions and scopes',
