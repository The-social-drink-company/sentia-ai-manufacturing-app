...
const Forecasting = () => {
  ...
  const runRate =
    chartData.length > 0
      ? Math.round(chartData.reduce((sum, p) => sum + (p.forecast ?? 0), 0) / chartData.length)
      : 0
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-center h-40">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Generating forecast from real sales data...
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <section className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Unable to generate real demand forecasts from available data sources
            </CardDescription>
          </CardHeader>
          <CardContent>
    ...
    <section className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Demand Forecasting</h1>
          <p className="text-sm text-muted-foreground">
            Real demand forecasts based on{' '}
            {metadata.dataSource === 'real_data' ? 'historical sales data' : 'Shopify sales trends'}{' '}
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge
            variant={metadata.dataSource === 'real_data' ? 'default' : 'secondary'}
            className="flex items-center gap-1"
          >
            {metadata.dataSource === 'real_data' ? (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Real forecasting models using {metadata.algorithm || 'ensemble methods'} - Generated{' '}
            {metadata.generatedAt ? new Date(metadata.generatedAt).toLocaleString() : 'recently'}
          </CardDescription>
        </CardHeader>
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <Badge variant={model.bias.startsWith('+') ? 'destructive' : 'default'}>
                {model.bias}
              </Badge>
            </div>
          </CardTitle>
                ...
                <tbody className="divide-y divide-border">
                  {chartData.map((row, index) => {
                    ...
                    const variancePercent =
                      actual !== undefined && forecast > 0 ? (variance / forecast) * 100 : null
      ...
      {metadata.aiInsights && metadata.aiInsights.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Intelligence-driven insights from demand pattern analysis
            </CardDescription>
          </CardHeader>
          <CardContent>
