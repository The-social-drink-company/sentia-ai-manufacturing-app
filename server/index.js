...
app.get('/dashboard', async (req, res) => {
    try {
        ...
        const executiveResponse = await fetch(`http://localhost:${PORT}/api/dashboard/executive`);
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .metric {
            ...
            margin-bottom: 10px; padding: 10px; background: rgba(255,255,255,0.05);
            border-radius: 8px;
        }
        .metric-value { font-weight: bold; font-size: 1.1rem; color: #28a745; }
        ...
        .status-dot {
            ...
            margin-right: 8px; background: #28a745; box-shadow: 0 0 10px #28a745;
        }
        .live-indicator {
            ...
            <div class="card">
                ...
                <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; margin-bottom: 10px;">
                    <p style="font-size: 0.9rem; line-height: 1.4;">
                        ${(executiveData.executive?.mcpInsights?.response || 'AI insights loading...').substring(0, 200)}...
                    </p>
                </div>
                <div class="metric">
                    <span>AI Confidence:</span>
                    <span class="metric-value">${((executiveData.executive?.mcpInsights?.confidence || 0) * 100).toFixed(1)}%</span>
                </div>
                <div class="metric">
