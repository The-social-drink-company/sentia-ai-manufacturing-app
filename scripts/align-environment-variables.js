...
function updateServiceEnvVars(service, environment) {
  ...
  Object.entries(CRITICAL_ENV_VARS).forEach(([key, value]) => {
  ...
  if (ENV_SPECIFIC[environment]) {
    Object.entries(ENV_SPECIFIC[environment]).forEach(([key, value]) => {
      if (existingVars.has(key)) {
        existingVars.get(key).value = value;
...
function createEnvFiles() {
  ...
  environments.forEach(env => {
    ...
    if (ENV_SPECIFIC[env]) {
      Object.entries(ENV_SPECIFIC[env]).forEach(([key, value]) => {
        content += `${key}=${value}\n`;
      });
    }
