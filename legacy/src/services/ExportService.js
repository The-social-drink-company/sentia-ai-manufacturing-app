...
class ExportService {
    ...
    if (data.workingCapitalData) {
      ...
      Object.entries(data.workingCapitalData.metrics).forEach(([key, value]) => {
        flattened.push({
          ...
          Metric: this.formatMetricName(key),
          Value: value,
          Unit: key.includes('so') || key.includes('io') || key.includes('po') || key === 'cashConversionCycle' ? 'days' : 'Â£'
        });
      });
    }
  ...
  formatDataForPDF(data) {
    ...
    if (data.financialData) {
      ...
      lines.push('---------------------');
      lines.push(`Revenue: Â£${data.financialData.revenue.current.toLocaleString()}`);
      lines.push(`Operating Margin: ${data.financialData.margins.operating}%`);
      lines.push(`Net Cash Flow: Â£${data.financialData.cashFlow.net.toLocaleString()}`);
      lines.push('');
    }
    ...
    if (data.workingCapitalData) {
      ...
      lines.push('------------------------');
      lines.push(`Working Capital: Â£${data.workingCapitalData.metrics.workingCapital.toLocaleString()}`);
