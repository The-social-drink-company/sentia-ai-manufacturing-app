    ...
    if (selectedSections.performanceKpis) {
      ...
      reportData.sections.performanceKpis = {
        ...
        data: kpiResponse.success ? [
          { label: 'Annual revenue', value: kpiResponse.data.annualRevenue.value, helper: kpiResponse.data.annualRevenue.helper },
          { label: 'Units sold', value: kpiResponse.data.unitsSold.value, helper: kpiResponse.data.unitsSold.helper },
          { label: 'Gross margin', value: kpiResponse.data.grossMargin.value, helper: kpiResponse.data.grossMargin.helper }
        ] : [],
...
const generateStockSummary = (stockData) => {
  ...
  const optimalStockItems = stockData.filter(item => item.status === 'Optimal' || (item.level >= 20 && item.level <= 80))
  ...
  return {
    ...
    keyInsight: lowStockItems.length > 0 
...
const generateProductSalesSummary = (salesData) => {
  ...
  const totalSales = salesData.reduce((sum, product) => sum + (product.sales || product.revenue || 0), 0)
...
const generateExecutiveSummary = (sections, dateRange) => {
  ...
  if (sections.performanceKpis) {
    insights.push('Key performance indicators showing positive trends across revenue and margin metrics')
  }
  ...
  if (sections.regionalContribution) {
    insights.push('Regional performance balanced across all markets with EU leading revenue contribution')
  }
