...
class AmazonIntegration {
  constructor(config = {}) {
    this.regions = {
      uk: {
        marketplaceId:
          config.UK_MARKETPLACE_ID || process.env.AMAZON_UK_MARKETPLACE_ID || 'A1F83G8C2ARO7P',
      ...
      usa: {
        marketplaceId:
          config.USA_MARKETPLACE_ID || process.env.AMAZON_USA_MARKETPLACE_ID || 'ATVPDKIKX0DER',
  ...
  generateSignature(method, url, headers, payload, region) {
    ...
    const kDate = crypto
      .createHmac(
        'sha256',
        `AWS4${this.regions[region === 'eu-west-1' ? 'uk' : 'usa'].secretAccessKey}`
      )
      .update(date)
      .digest()
  ...
  async getOrders(options = {}) {
    ...
    try {
      const [ukOrders, usaOrders] = await Promise.allSettled([
        this.makeRequest(
          'uk',
          `/orders/v0/orders?MarketplaceIds=${this.regions.uk.marketplaceId}&${params}`
        ),
        this.makeRequest(
          'usa',
          `/orders/v0/orders?MarketplaceIds=${this.regions.usa.marketplaceId}&${params}`
        ),
      ])
  ...
  async getCatalogItems(options = {}) {
    ...
    try {
      const [ukCatalog, usaCatalog] = await Promise.allSettled([
        this.makeRequest(
          'uk',
          `/catalog/2022-04-01/items?${params}&marketplaceIds=${this.regions.uk.marketplaceId}`
        ),
        this.makeRequest(
          'usa',
          `/catalog/2022-04-01/items?${params}&marketplaceIds=${this.regions.usa.marketplaceId}`
        ),
      ])
  ...
  async getInventorySummary(options = {}) {
    const { granularityType = 'Marketplace', granularityId, marketplaceIds, skus } = options
    ...
    try {
      const [ukInventory, usaInventory] = await Promise.allSettled([
        this.makeRequest(
          'uk',
          `/fba/inventory/v1/summaries?${params}&marketplaceIds=${this.regions.uk.marketplaceId}`
        ),
        this.makeRequest(
          'usa',
          `/fba/inventory/v1/summaries?${params}&marketplaceIds=${this.regions.usa.marketplaceId}`
        ),
      ])
      ...
      const result = {
        uk: {
          ...
          summaries:
            ukInventory.status === 'fulfilled'
              ? ukInventory.value.payload?.inventorySummaries || []
              : [],
        ...
        usa: {
          ...
          summaries:
            usaInventory.status === 'fulfilled'
              ? usaInventory.value.payload?.inventorySummaries || []
              : [],
    ...
    try {
      ...
      const result = {
        uk: {
          ...
          events:
            ukFinancials.status === 'fulfilled'
              ? ukFinancials.value.payload?.FinancialEventGroupList || []
              : [],
        ...
        usa: {
          ...
          events:
            usaFinancials.status === 'fulfilled'
              ? usaFinancials.value.payload?.FinancialEventGroupList || []
              : [],
