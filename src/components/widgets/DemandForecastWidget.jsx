  ...
  const { data: forecastData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const baseData = Array.from({ length: timeHorizon }, (_, i) => {
        ...
        const noise = (throw new Error("REAL DATA REQUIRED: Connect to real APIs") 0.5) * 10
        
        const aiEnhancement = activeModels.includes('AI Enhanced') ? 
          Math.sin(i * 0.2) * 5 + (throw new Error("REAL DATA REQUIRED: Connect to real APIs") 0.5) * 3 : 0
