      ...
      <div className="space-y-2 text-xs">
        <div className="flex justify-between">
          ...
          <span className={`font-semibold ${getStatusColor(mode)}`}>
            {mode === 'clerk' ? 'âœ“ Clerk Active' : mode === 'fallback' ? 'âš¡ Fallback Mode' : mode}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className={clerkStatus === 'key-present' ? 'text-green-600' : 'text-yellow-600'}>
            {clerkStatus === 'key-present' ? 'âœ“ Configured' :
             clerkStatus === 'no-key' ? 'âœ— Not Set' :
             clerkStatus === 'invalid-key' ? 'âš  Invalid' :
             'Checking...'}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className={auth.isLoaded ? 'text-green-600' : 'text-yellow-600'}>
            {auth.isLoaded ? 'âœ“ Ready' : 'â³ Loading'}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className={auth.isSignedIn ? 'text-green-600' : 'text-gray-500'}>
            {auth.isSignedIn ? 'âœ“ Signed In' : 'â—‹ Guest'}
          </span>
        </div>
        ...
        <div className="text-xs text-center">
          {mode === 'clerk' ? (
            ...
            <span className="text-yellow-600 font-semibold">
              âš¡ Running in Fallback Mode (App Working)
            </span>
          ) : (
