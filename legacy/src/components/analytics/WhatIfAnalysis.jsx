...
const generateCalendarAwareRecommendations = (scenarios, calculations) => {
  ...
  if (cccResults && cccResults.cashConversionCycle > ((cccResults.benchmarks?.targetCCC || 0)) + 10) {
    recommendations.push({
      ...
      impact: `Potential cash release: Â£${Math.round((cccResults.cashConversionCycle - 45) * 40000000 / 365).toLocaleString()}`
    });
  }
  ...
  if (context && [9, 10].includes(context.currentMonth)) {
    recommendations.push({
      ...
      impact: `Additional WC needed: Â£${Math.round(workingCapital * 0.2).toLocaleString()}`
    });
  }
