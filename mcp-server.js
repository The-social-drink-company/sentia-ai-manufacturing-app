...
const tools = {
  ...
  'unified-api-call': async (params) => {
    ...
    const serviceConfigs = {
      xero: { baseUrl: 'https://api.xero.com/api.xro/2.0', auth: 'Bearer ' + process.env.XERO_TOKEN },
      shopify: { baseUrl: 'https://your-shop.myshopify.com/admin/api/2024-01', auth: process.env.SHOPIFY_TOKEN },
      amazon: { baseUrl: 'https://sellingpartnerapi-na.amazon.com', auth: process.env.AMAZON_TOKEN },
      unleashed: { baseUrl: 'https://api.unleashedsoftware.com', auth: process.env.UNLEASHED_TOKEN }
    };
  ...
  'cash-runway-analysis': async (params) => {
    const { cashBalance, burnRate, revenue, scenarios = ['base', 'optimistic', 'pessimistic'] } = params;
...
async function calculateOptimalInventory(sku, current, forecast, constraints) {
  ...
  return Math.max(constraints.min || 0, Math.min(constraints.max || Infinity, forecast + safetyStock));
}
...
function generateARIMAForecast(data, horizon) {
  return Array(horizon).fill(0).map((_, i) => data[data.length - 1] * (1 + Math.random() * 0.1));
}

function generateLSTMForecast(data, horizon) {
  return Array(horizon).fill(0).map((_, i) => data[data.length - 1] * (1 + Math.random() * 0.08));
}

function generateProphetForecast(data, horizon) {
  return Array(horizon).fill(0).map((_, i) => data[data.length - 1] * (1 + Math.random() * 0.12));
}
...
const httpServer = app.listen(PORT + 1, () => {
  ...
  Environment: ${process.env.NODE_ENV || 'development'}
  Providers: ${Object.keys(providers).filter(p => providers[p]).join(', ')}
