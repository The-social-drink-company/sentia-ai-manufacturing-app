  ...
  generateRecommendations(dso, dio, dpo) {
    ...
    if (dso > this.config.targetDSO + 5) {
      recommendations.push({
        ...
        impact: `Could free up Â£${Math.round((dso - this.config.targetDSO) * 40000000 / 365).toLocaleString()} in working capital`
      });
    }
    ...
    if (dio > this.config.targetDIO + 5) {
      recommendations.push({
        ...
        impact: `Could free up Â£${Math.round((dio - this.config.targetDIO) * 26000000 / 365).toLocaleString()} in working capital`
      });
    }
    ...
    if (dpo < this.config.targetDPO - 5) {
      recommendations.push({
        ...
        impact: `Could improve cash flow by Â£${Math.round((this.config.targetDPO - dpo) * 26000000 / 365).toLocaleString()}`
      });
    }
    ...
    if (ccc > targetCCC + 10) {
      recommendations.push({
        ...
        impact: `Total optimization potential: Â£${Math.round((ccc - targetCCC) * 40000000 / 365).toLocaleString()}`
      });
    }
