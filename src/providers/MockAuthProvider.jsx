...
const MockAuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      setIsLoaded(true)
      logInfo('[MockAuthProvider] Mock auth initialized', { email: state.user?.email })
    }, 50)
  ...
  const login = useCallback(async (details = {}) => {
    ...
    logInfo('[MockAuthProvider] Signed in user', { email: nextUser.email, role: nextUser.role })
    setState({ user: nextUser, isAuthenticated: true })
    return nextUser
  }, [])

  const logout = useCallback(async () => {
    logInfo('[MockAuthProvider] Signed out user')
