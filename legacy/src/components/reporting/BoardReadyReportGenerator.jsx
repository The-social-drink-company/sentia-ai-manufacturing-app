...
const BoardReadyReportGenerator = ({ dashboardData, companyInfo }) => {
  ...
  const generateExecutiveSummary = () => {
    ...
    return {
      overview: `Our working capital analysis reveals significant opportunities to optimize cash flow and support strategic growth initiatives. Current cash runway extends ${cashRunway?.summary?.cashRunwayDays || null} days, with potential to unlock Â£${((optimization?.summary?.cashUnlockPotential || 0) / 1000).toFixed(0)}K through targeted working capital improvements.`,
      
      keyFindings: [
        `Cash runway: ${cashRunway?.summary?.cashRunwayDays || null} days with current burn rate`,
        `Working capital optimization potential: Â£${((optimization?.summary?.cashUnlockPotential || 0) / 1000).toFixed(0)}K`,
        `Growth funding requirement: Â£${((fundingRequirements?.summary?.totalFundingRequired || 0) / 1000).toFixed(0)}K for planned expansion`,
      ...
      strategic: [
        {
          ...
          impact: 'Provide Â£500K-Â£1M funding flexibility',
          timeline: '8-12 weeks',
              ...
              <Card>
                <CardHeader className="bg-gradient-to-r from-blue-600 to-blue-800 text-white">
                  ...
                  <CardDescription className="text-blue-100">
                    Generated for {reportConfig.audience.replace(/_/g, ' ')} â€¢ {format(new Date(), 'MMMM dd, yyyy')}
                  </CardDescription>
                </CardHeader>
              </Card>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-blue-900">
                              Â£{((generatedReport.keyMetrics.financial?.currentCash || 0) / 1000).toFixed(0)}K
                            </p>
                          </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-green-900">
                              Â£{((generatedReport.keyMetrics.financial?.cashUnlockPotential || 0) / 1000).toFixed(0)}K
                            </p>
                          </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-purple-900">
                              Â£{((generatedReport.keyMetrics.financial?.workingCapital || 0) / 1000).toFixed(0)}K
                            </p>
                          </div>
